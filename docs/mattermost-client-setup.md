...
- Primary URL: `https://mattermost.lab.jamilshaikh.in`
...
2. Enter `Server URL` as `https://mattermost.lab.jamilshaikh.in`.
...
2. Add server URL: `https://mattermost.lab.jamilshaikh.in`.
...
- Confirm the URL is exactly `https://mattermost.lab.jamilshaikh.in` (not `http://`).
- If your laptop browser shows a certificate warning on that URL, the app will usually stay stuck at validation.
- Trust the homelab root CA on your laptop and iPhone, then retry.
...
- Confirm iPhone can resolve and reach `mattermost.lab.jamilshaikh.in` on your network.
