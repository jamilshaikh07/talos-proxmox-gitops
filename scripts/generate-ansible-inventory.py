...
def get_ip_from_dns(hostname):
    """Get IP from DNS lookup when Terraform did not provide a planned IP."""
    try:
        result = subprocess.run(
    ...
    print(
        f"Warning: No planned_ip or DNS record found for {hostname}; inventory will contain 'unknown' for this node.",
        file=sys.stderr,
    )
    return 'unknown'
...
Examples:
  ...
  %(prog)s --baremetal-ip 10.20.0.45 # Add/update a bare metal node with auto disk detection
    ...
    cp_ip = cp_data.get('planned_ip') or get_ip_from_dns(cp_name)
    ...
    for worker_name in sorted(worker_vms.keys()):  # Sort for consistent order
        ...
        worker_ip = worker_data.get('planned_ip') or get_ip_from_dns(worker_name)
        worker_mac = worker_data.get('planned_mac_address') or worker_data.get('mac_address', '')
    ...
    ansible_vars = (
        f"---\n"
        f"# Talos Cluster Configuration Variables\n"
        f"# VM workers are AUTO-GENERATED from Terraform outputs\n"
        f"# Bare metal workers (baremetal_workers) are manually maintained - DO NOT DELETE\n"
        f"# Generated by: scripts/generate-ansible-inventory.py\n\n"
        f"# Cluster configuration\n"
        f"cluster_name: \"homelab-cluster\"\n"
        f"cluster_endpoint: \"https://{cp_ip}:6443\"  # Control plane IP\n\n"
        f"# Default install disk (used if not specified per node)\n"
        f"default_install_disk: \"/dev/sda\"\n\n"
        f"# Default Longhorn disk (used for storage on all nodes)\n"
        f"default_longhorn_disk: \"/dev/sdb\"\n\n"
        f"# Talos node configuration\n"
        f"# - install_disk: optional, defaults to default_install_disk\n"
        f"# - longhorn_disk: optional, dedicated disk for Longhorn storage (defaults to default_longhorn_disk)\n"
        f"talos_nodes:\n"
        f"  control_plane:\n"
        f"    hostname: \"{cp_name}\"\n"
        f"    ip: \"{cp_ip}\"\n"
        f"    vmid: {cp_vmid}\n"
        f"    mac_address: \"{cp_data.get('planned_mac_address') or cp_data.get('mac_address', '')}\"\n"
        f"    longhorn_disk: \"/dev/sdb\"  # 500GB for Longhorn\n"
        f"  # VM workers (from Terraform)\n"
        f"  workers:\n"
    )
    ...
    for worker in workers:
        ansible_vars += (
            ...
            f"      ip: \"{worker['ip']}\"\n"
            f"      vmid: {worker['vmid']}\n"
            f"      mac_address: \"{worker['mac_address']}\"\n"
            f"      longhorn_disk: \"/dev/sdb\"  # 500GB for Longhorn\n"
        )
...
"""
    ...
    for worker in workers:
        mac_suffix = f", MAC {worker['mac_address']}" if worker['mac_address'] else ""
        print(f"     - {worker['hostname']} ({worker['ip']}{mac_suffix})")
